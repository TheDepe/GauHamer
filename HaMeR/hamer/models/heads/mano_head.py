...
class MANOTransformerDecoderHead(nn.Module):
    ...
    THIS IS WHERE THE CAM ISSUE IS ARISING FROM
    """

    def __init__(self, cfg):
        # INFO: LOADS CONFIG FROM .CACHE IN DOWNLOADED DATA
        super().__init__()
        self.cfg = cfg
        ...
        self.npose = npose
        self.input_is_mean_shape = cfg.MODEL.MANO_HEAD.get('TRANSFORMER_INPUT', 'zero') == 'mean_shape' # false
    ...
    def forward(self, x, **kwargs):
        ...
        pred_cam = init_cam
        #pred_cam = torch.tensor([[-0.5, 0.5, 0.5]]).to(pred_betas.device) # Hacky work around
        pred_hand_pose_list = []
        pred_betas_list = []
        pred_cam_list = []
