...
def readCamerasInfosMIRAGE(rgb_paths, poses, intrins, distcoeff, camera_idx, random_background_color=True, bg_color=0, tight_crop=False):
    cam_infos = []
    ...
    for i, c2w in enumerate(poses):
        ...
        T = c2w[3, :3] # right column # THIS IS NOW CAM CENTER FOR SOME REASON
        ...
        full_rgba = imageio.imread(image_path)
        image = full_rgba[:,:,:3]
        h, w, _ = image.shape
        if distcoeff is not None:
            cur_dist = distcoeff[i]
        ...
        if np.max(mask) == 0:
            mask = Image.fromarray(mask)  # Keep mask as is if max is zero
        else:
            mask = Image.fromarray(mask * (255 // np.max(mask)))
